...
#     print('Qr: %.2e, nominal tau %.2e, 1/tau %.2e'%(Qr, nominal_tau, 1./nominal_tau))
    ...
    while step < len(nqp_timestream):
        ...
        if step % sample_rate_factor == 1:
            ...
            next_Vin = starting_Vin * (1 + acc)
            # enforce non negative and non infinite
            min_Vin = 0
            max_Vin = 1e-1
            if next_Vin > min_Vin and next_Vin < max_Vin:
                Vin = next_Vin
...
def single_resonance_iterator_with_fb_2(res, carrier_freq_timestream,
                           nqp_timestream=None,
                           starting_Vin=1e-5, setpoint=None, theta_rot=0, 
                           delay_nsamps=5, Icoeff=0,
                          watcher_span = 100e3, nwatchers=1000, 
...
#     print('Qr: %.2e, nominal tau %.2e, 1/tau %.2e'%(Qr, nominal_tau, 1./nominal_tau))
    ...
    # set up the feedback parameters
    meas_buf = deque([setpoint*starting_Vin]*delay_nsamps, maxlen=delay_nsamps) # let's just use a single delay for now
    acc = 0
    ...
    while step < len(nqp_timestream):
        ...
        if step % sample_rate_factor == 1:
            ...
            Vout_delayed = meas_buf[0]
            ttt_sys = 1./system_fs
            # acc += (Vout_delayed.imag - setpoint.imag * Vin) * (ttt_sys) * Icoeff
            acc += (setpoint.imag*Vin - Vout_delayed.imag) * (ttt_sys) * Icoeff 
            next_Vin = starting_Vin * (1 + acc)
            # enforce non negative and non infinite
            min_Vin = 0
            max_Vin = 1e-1
